...
function postCleanEffect(effect3) {
  if (effect3._deps.length !== effect3._depsLen) {
    for (let i = effect3._depsLen; i < effect3._deps.length; i++) {
      cleanDepEffect(effect3._deps[i], effect3);
    }
  }
  effect3._deps.length = effect3._depsLen;
}
...
function trackEffect(effect3, dep) {
  if (dep.get(effect3) !== effect3._trackId) {
    ...
    const oldDep = effect3._deps[effect3._depsLen];
    if (oldDep !== dep) {
      if (oldDep) {
        cleanDepEffect(oldDep, effect3);
      }
      effect3._deps[effect3._depsLen++] = dep;
    } else {
      effect3._depsLen++;
    }
  }
...
function triggerEffect(dep) {
  for (const effect3 of dep.keys()) {
    if (!effect3.dirty) effect3.dirty = true;
    if (!effect3._running) {
      effect3?.scheduler();
    }
  }
...
function trackRefValue(ref3) {
  if (activeEffect) {
    trackEffect(
      ...
      ref3._deps = createDep("value", () => ref3.dep = void 0)
    );
  }
}
function triggerRefValue(ref3) {
  let dep = ref3._deps;
  if (dep) {
    triggerEffect(dep);
  }
...
function trackEffect2(effect22, dep, debuggerEventExtraInfo) {
  var _a;
  if (dep.get(effect22) !== effect22._trackId) {
    ...
    if (true) {
      (_a = effect22.onTrack) == null ? void 0 : _a.call(effect22, extend({ effect: effect22 }, debuggerEventExtraInfo));
    }
  }
}
...
function triggerEffects(dep, dirtyLevel, debuggerEventExtraInfo) {
  ...
  for (const effect22 of dep.keys()) {
    ...
    if (effect22._shouldSchedule && (tracking != null ? tracking : tracking = dep.get(effect22) === effect22._trackId)) {
      if (true) {
        (_a = effect22.onTrigger) == null ? void 0 : _a.call(effect22, extend({ effect: effect22 }, debuggerEventExtraInfo));
      }
      effect22.trigger();
...
var createDep2 = (cleanup, computed3) => {
  const dep = /* @__PURE__ */ new Map();
  dep.cleanup = cleanup;
...
function track(target, type, key) {
  if (shouldTrack && activeEffect2) {
    ...
    let dep = depsMap.get(key);
    if (!dep) {
      depsMap.set(key, dep = createDep2(() => depsMap.delete(key)));
    }
    trackEffect2(
      activeEffect2,
      dep,
...
var BaseReactiveHandler = class {
  ...
  get(target, key, receiver) {
    ...
    if (key === "__v_isReactive") {
      ...
      if (receiver === (isReadonly2 ? isShallow2 ? shallowReadonlyMap : readonlyMap : isShallow2 ? shallowReactiveMap : reactiveMap2).get(target) || // receiver is not the reactive proxy, but has the same prototype
...
var mutableHandlers2 = /* @__PURE__ */ new MutableReactiveHandler();
...
function get(target, key, isReadonly2 = false, isShallow2 = false) {
  ...
  const wrap = isShallow2 ? toShallow : isReadonly2 ? toReadonly : toReactive2;
...
function doWatch(source, cb, {
  ...
  const effect3 = new ReactiveEffect2(getter, NOOP, scheduler);
...
function getComponentPublicInstance(instance) {
  if (instance.exposed) {
    return instance.exposeProxy || (instance.exposeProxy = new Proxy(proxyRefs2(markRaw(instance.exposed)), {
      get(target, key) {
        if (key in target) {
          return target[key];
...
function initCustomFormatter() {
  ...
  function isKeyOfType(Comp, key, type) {
    ...
    if (isArray(opts) && opts.includes(key) || isObject2(opts) && key in opts) {
      return true;
    }
...
function watchEffect(source, options = {}) {
  return doWatch2(source, null, options);
}
function traverse2(source, depth, currentDepth = 0, seen = /* @__PURE__ */ new Set()) {
  ...
  for (let key in source) {
    traverse2(source[key], depth, currentDepth, seen);
  }
  return source;
}
function doWatch2(source, cb, { deep, immediate }) {
  const reactiveGetter = (source2) => traverse2(source2, deep === false ? 1 : void 0);
  let getter;
